...
private static class Thread_Runnable_Server implements Runnable {
	...
	byte[] Buffer_Read = new byte[S_Checks];
      try {
        int Connection_Map_Len = CM.size();
            	int Current_Port = Current_Node.Get_Port_Number();

        ServerSocket os = new ServerSocket(Current_Port);

        System.out.println("given size " + Connection_Map_Len);
        int Peer_Map_Len = MP.size();
        System.out.println("present connection " + Connection_Map_Len);
        int a;
        for ( a=0; Connection_Map_Len < Peer_Map_Len - 1;) {
          a++;
          System.out.println("Establishing the connection");
          Socket conn = os.accept();

          DataInputStream ISC = new DataInputStream(conn.getInputStream());
          System.out.println("Read the conn");
          ISC.readFully(Buffer_Read);
          StringBuilder Message_Hand_Shake = new StringBuilder();
          byte[] BHT = Helper_Functions.Array_Byte_Copy(Buffer_Read, 0, 28);
          Message_Hand_Shake.append(new String(BHT));
          ...
          BufferedWriter writer = new BufferedWriter(new FileWriter(File_Log.getAbsolutePath(), true));
          CM.put(Peer_ID, new Adjacent_Connection_Node(conn, Peer_ID));
          
          Connection_Map_Len = CM.size();
          Peer_Map_Len = MP.size();
  ...
  static Read_Common_config Read_Conf;
  static Data_of_Common_config Data_of_Common_config;
  static LinkedHashMap<Integer, Peer_Node> MP;
  ...
  private static class Adjacent_Connection_Node {
    ...
    public byte[] Get_Chunk_File(int Piece_Index, byte[] piece) {
      int Key_Index = 0;
      int a;
      byte[] VP = new byte[4 + piece.length];

      byte[] Key_Bytes = ByteBuffer.allocate(4).putInt(Piece_Index).array();
      
      for (a=0;a < Key_Bytes.length;a++) {
		 VP[Key_Index++] = Key_Bytes[a];
      }
      for (a=0;a< piece.length;a++) {
	        VP[Key_Index++] = piece[a];
      }
      byte[] pac = null;
      try {
          pac =  Pack_Gen((5 + piece.length), Types_of_Messages.Piece_Char(), VP);
    ...
    public byte[] Pack_Gen(int ln, char type, byte[] VP) throws Wrong_Packet_Exception {
    ...
    public void MSR(int chunkKey_Index) {
    	try {
            DataOutputStream cos = new DataOutputStream(conn.getOutputStream());
            cos.flush();
            
            byte[] VP = ByteBuffer.allocate(4).putInt(chunkKey_Index).array();
            byte[] Message_Request = null;
            try {
                Message_Request = Pack_Gen(5, Types_of_Messages.Requested_Char(), VP);
            } catch(Wrong_Packet_Exception ex) {
            	System.out.println(ex.getMessage());
            	System.exit(0);
            }
            cos.write(Message_Request);
            cos.flush();
    	}  catch (IOException e) {
    ...
    public void MSP(int Key_Index) {
    	...
    	try {
            ...
            cos.write(Get_Chunk_File(Key_Index, Present_FC[Key_Index]));
            cos.flush();
            cos.flush();
            
    	}  catch (IOException exception) {
    		System.out.println("There was mistakes/errors while sending piece packets");
    		
    	} finally {
    		System.out.println("Piece packets are sent completely..");
    	}
    	
    }
    
    public boolean Thread_Scope_Func() {
    	System.out.println("Thread reached here");
    	return true;
    }
    public void MSH(int Piece_Index) {
    	...
    	try {
            ...
            /*
              scope checking here...
             */
            boolean vvv = Thread_Scope_Func();
            if (vvv == true)
            {
            	cos = new DataOutputStream(conn.getOutputStream());
            }
            cos.flush();
            byte[] VP = ByteBuffer.allocate(4).putInt(Piece_Index).array();
            byte[] Message_Have = null;
            try {
                Message_Have = Pack_Gen(5, Types_of_Messages.Have_Char(), VP);
            } catch(Wrong_Packet_Exception ex) {
            	System.out.println(ex.getMessage());
            	System.exit(0);
            }
            cos.write(Message_Have);
            cos.flush();
    ...
    private boolean D_All(int p, int q, String r) {
    	if (r.equals("")) return true;
        int i = r.length();
        if ( q >= i || p < 0) return false;
        if (q < p) return false;
        while(p <= q) {
            if (! isD(r.charAt(p))) return false;
            p++;
        }
        return true;
    }
    ...
    public void Interested_Message_Send() {
    	...
    	try {
            ...
            try {
                Message_Interested = Pack_Gen(1, Types_of_Messages.Interested_Char(), null);
            } catch(Wrong_Packet_Exception ex) {
            	System.out.println(ex.getMessage());
            	System.exit(0);
            }
            cos.write(Message_Interested);
    ...
    public void Bit_Field_Message_Send() {
    	 ...
    	 */
    	 try{
            ...
            byte[] VP = new byte[New_Message_Length - 1];
            int Key_Index = 0;
            int a,b;
            ...
            int num = Bit_Field[a];
              byte[] eachNumberByteArray = ByteBuffer.allocate(4).putInt(num).array();
	            ...
	            VP[Key_Index++] = Each_Byte;
	          }
            }
            byte[] Bit_Field_Message = null;
            
            try {
                Bit_Field_Message = Pack_Gen(New_Message_Length, Types_of_Messages.Bit_Field_Char(), VP);
            } catch(Wrong_Packet_Exception ex) {
            	System.out.println(ex.getMessage());
            	System.exit(0);
            }
            cos.write(Bit_Field_Message);
            cos.flush();
    	} catch(IOException exception){
    ...
    public void Not_Interested_Message_Send() {
    	try {
            ...
            try {
                Message_Not_Interested = Pack_Gen(1, Types_of_Messages.Not_Interested_Char(), null);
            } catch(Wrong_Packet_Exception ex) {
            	System.out.println(ex.getMessage());
            	System.exit(0);
            }
            
            cos.write(Message_Not_Interested);
            cos.flush();
    	} catch (IOException exception) {
    ...
    public void Choke_Message_Send() {
    	try {
            ...
            try {
                Choke_Message = Pack_Gen(1, Types_of_Messages.Choke_Char(), null);
           } catch(Wrong_Packet_Exception ex) {
            	System.out.println(ex.getMessage());
            	System.exit(0);
            }
            
            cos.write(Choke_Message);
            cos.flush();
    ...
    public void UnChoke_Message_Send() {
    	...
    	try {
            ...
            try {
                unChoke_Message = Pack_Gen(1, Types_of_Messages.UNChoke_Char(), null);
           } catch(Wrong_Packet_Exception ex) {
            	System.out.println(ex.getMessage());
            	System.exit(0);
            }
    ...
    public void Completed_Check(int Receiver_Chunk_File_Index) {
      ...
      if (Total_Pieces == Bit_Fields_Curr_Node.length) {
        ...
        int b,c;
        for ( b=0; b < Present_FC.length; b++) {
        	for ( c=0; c < Present_FC[b].length;c++) {
        		byte pq = Present_FC[b][c];
        		In_Bytes_New_File[Temp_Index++] = pq;
	          }
        }
        try {
          Random random = new Random();
          //int i = random.nextInt(5000000);
          String File_Final_Path = Read_Conf.Get_Path_Of_Root() + "/peer_" + Cur_Node_Id + "/" + (FMName + "." + FormatF);
          ...
          int b;
          	for ( b=0; b < Present_FC[Receiver_Chunk_File_Index].length;b++) {
          		byte pq = Present_FC[Receiver_Chunk_File_Index][b];
          		In_Bytes_New_File[Temp_Index++] = pq;
  	      }
          ...
          try {
              String File_Final_Path = Read_Conf.Get_Path_Of_Root() + "/peer_" + Cur_Node_Id + "/" + ("chunk_" + FMName + "_" + Receiver_Chunk_File_Index + "." + FormatF);
              File_To_Write(File_Final_Path, In_Bytes_New_File);
  ...
  static byte[][] Present_FC;
  static int Complete_Peer_Files = 0;
  static File PresentDirectory;
  private static ConcurrentHashMap<Integer, Adjacent_Connection_Node> CM;
  private static File File_Log;
  private static String FormatF;
    ...
    public void run() {
      synchronized (this) {
        ...
        try {
          ...
          while (Complete_Peer_Files < MP.size()) {
            ...
            if (Message_Type == Types_of_Messages.Bit_Field_Char()) {
               ...
               */
              Peer_Node Peer_Object_Connected = MP.get(peer.Get_Peer_ID());
             ...
             * Tells I don't want anything from this connection
              * */
...

              {
                ...
                Peer_Node Peer_Object_Connected = MP.get(peer.Get_Peer_ID());
                Present_FC[Receiver_Chunk_File_Index] = new byte[Message.length - 4];

                int b = 0;
                int a;
                for (a = 4; a < Message.length; a++) {
	                byte[] Current_File_Row = Present_FC[Receiver_Chunk_File_Index];
                 ...
                 */
                for (int connect: CM.keySet()) {
                  Adjacent_Connection_Node pac = CM.get(connect);
                  pac.MSH(Receiver_Chunk_File_Index);
                }
              }
  ...
  private static class Thread_Runnable_Client implements Runnable {
	    ...
	    public void run() {
	      try {
	        byte[] Pipe_Response;
	        for (Entry<Integer, Peer_Node> entry : MP.entrySet()) {
	          int key_Val = entry.getKey();
	          if (key_Val == Cur_Node_Id) {
	            break;
	          }
	          System.out.println("Client:" + key_Val);
	          Peer_Node Peer_Connection = MP.get(key_Val);
	          ...
	          try {
	            ...
	            System.out.println("error to running Logs operation ");
	              ...
	              CM.put(key_Val, new Adjacent_Connection_Node(clientSocket, key_Val));
	              System.out.println("Client: " + key_Val + " From Server: Handshake packet received " + Peer_Connection.Get_Host_Name()
	                  ...
	                  + " connections Updated" + CM.size() + "/" + (MP.size() - 1)
  ...
  public static void Common_Config_File_Initializing() throws IOException {
    ArrayList<String> Common_Config_File_Parse = Read_Conf.Parse_Com_Config_File("Common.cfg");
  ...
  public static void PeerInfo_Config_File_Initializing() throws IOException {
    ArrayList<String> PeerInfo_Config_File_Parse = Read_Conf.Parse_Com_Config_File("PeerInfo.cfg");
  ...
  public static void Peer_Directory_Initialize() {
    PresentDirectory = new File("peer_" + Cur_Node_Id);
    if (!PresentDirectory.exists()) {
      PresentDirectory.mkdir();
    }
  }
  /*
  ...
  public static void File_Chunks_Write() throws IOException {
    ...
    String To_Path = Read_Conf.Get_Path_Of_Root() + Constants.Get_Transfer_File();
...

    {
      ...
      for (; cnt < File_Size;) {
        if (cnt + Size_of_Chunk <= File_Size)
          Present_FC[Key_Index] = Helper_Functions.Array_Byte_Copy(Bytes_File, cnt, cnt + Size_of_Chunk);
        else
          Present_FC[Key_Index] = Helper_Functions.Array_Byte_Copy(Bytes_File, cnt, File_Size);
        Current_Node.Update_Num_Of_Pieces();
        cnt += Size_of_Chunk;
        Key_Index++;
      }
  ...
  public static void Chunks_Calculate() throws IOException {
    ...
    Present_FC = File_Chunks_Initialize(Chunks_Round_Off);
    /*
  ...
  public static void main(String[] args) throws IOException {
     ...
     * config file reader
     */
    Read_Conf = new Read_Common_config();
    ...
    Current_Node = MP.get(Cur_Node_Id);
  ...
  private static class Upper_Runnable implements Runnable {
    ...
    public List<Integer> Interested_Connection_Fetch(List<Integer> Connect) {
        ...
        Adjacent_Connection_Node Connection_Temp = CM.get(connects);
	...
	* */
	...
	public List<Integer> Interested_Acc_To_Dload_Rate_Fetch(List<Integer> Connect) {
        ...
        for (int Peers : Connect) {
            Adjacent_Connection_Node Connection_Object = CM.get(Peers);
      ...
      while (Complete_Peer_Files < MP.size()) {
        ...
        if (Current_Node.Get_Has_File() == 1) {
          ...
          if (Peer_Interested.size() <= 0) {
            ...
            if (Interested_Msg_Length <= Preferred_Neighbours) {
                ...
                for ( a=0; a < Interested_Msg_Length;a++) {
                	...
                	Adjacent_Connection_Node Connection_Temp = CM.get(Peers);
                ...
                int Peer_ID_Temp = Preferred_Neighbors[a];
                  Adjacent_Connection_Node Connection_Object = CM.get(Peer_ID_Temp);
          ...
          if (Peer_Interested.size() <= Preferred_Neighbour_Size) {
            ...
            ValComp bb =  new ValComp(Maps);
            TreeMap<Integer,Double> Map_Sorted = new TreeMap<Integer,Double>(bb);
            int a,b,c;
            for( a=0; a<Peer_Interested.size();a++){
              Maps.put(Peer_Interested.get(a),CM.get(Peer_Interested.get(a)).Download_Speed_Fetch());
            }
...
static class ValComp implements Comparator<Integer> {
    ...
    public ValComp(Map<Integer, Double> Base_Value) {
  ...
  private static class OUpper_Runnable implements Runnable {
	...
	public List<Integer> Interested_Connection_Fetch(List<Integer> Connects) {
        ...
        Adjacent_Connection_Node Connection_Temp = CM.get(connect);
          if (Connection_Temp.Is_Interested()) {
    ...
    public void run() {
      while (Complete_Peer_Files < MP.size()) {
        ...
        if (Interested_Msg.size() > 0) {
          ...
          Adjacent_Connection_Node Connection_Random = CM.get(Connection_Random_ID);
